...
//						jp_f.addPhon(puk, C.P);
	...
	private final PhonPanels gps=new PhonPanels();
	private final JPanel gpsPanel;
	/**dock north 始终将此Panel至于顶上方，并独占**/
	public final String gsC_migl="growx,wrap";
	...
	/**不要求 在 EDT中调用**/
	public synchronized void showWord(final Word aw){
		removeAll();
	...
	/**不要求 在 EDT中调用**/
	public synchronized void addPhon(Phon ap, final C ac) {
		PhonPanel pp=null;
...
//			return;
	 ...
	 * </p>不要求 在 EDT中调用**/
	public synchronized void addVoice(Voice av, C ac) {
