...
public class KingSoft extends AbstractGetEntry {
	...
	/*********** Test End *********/
	public final DBEntry gDbE;
	public KingSoft(ResultsPanel aRP) {
		super(aRP);
		gDbE=new DBEntry(aRP);
	}
	
	public synchronized Entry byWord(String ast) {
		stop();
		T.sleep(50);
		...
		if (!ast.equals(e.word.stW.get())) {
			e.word.stRedirectFrom=ast;
		}
		//////////////////////////////////////
		...
		for (Element ele : esSentces) {
			...
			stUrl = Regex.get_first_text(stReUrl, ele.html()
					.toLowerCase());
			s.v.stM.set(stUrl);
			e.sentces.add(s);
