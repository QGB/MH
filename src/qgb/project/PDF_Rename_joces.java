...
public class PDF_Rename_joces {
	...
	public static String get_name(int ai_number) throws Exception {
		...
		//st_html = T.read_st("13827.shtml", "UTF-8");
		...
		//T.print(st_re);
		st_pdf_name=Regex.get_first_text(st_re, st_html).trim();
		
		st_re = "(?<=<td class=\"J_ZhaiYao_ZuoZhe\" valign=\"top\">\\n).*(?=</td>)";
		//T.print(st_re);
		st_pdf_name = ai_number+st_pdf_name +"_"+ Regex.get_first_text(st_re, st_html).trim();
		
		//T.print(st_pdf_name);
		st_pdf_name=F.name(st_pdf_name)+".pdf";
		//T.print(st_pdf_name);
		
		T.write("shtml/"+ai_number+".shtml",st_html,CharsetName.gst_utf8);
		return st_pdf_name;
	}
}
